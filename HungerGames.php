...
class HungerGames extends PluginBase implements Listener {
    ...
    public $prefix = TextFormat::GRAY . "" . TextFormat::GRAY . "Hunger" . TextFormat::GRAY . "Games" . TextFormat::RESET . TextFormat::GRAY . "";
	public $mode = 0;
	public $arenas = array();
	public $currentLevel = "";
	...
	{
		  $this->getLogger()->info(TextFormat::RED . "HungerGames by DragonPlayzMC");
		...
		if($lang->get("en")==null)
		{
			...
			$messages["cannotjoin"] = "You cannot join this game.";
			$messages["seconds"] = "§fSeconds to be the Game will be started!";
			$messages["won"] = "§7Won in the game §8!";
			$messages["deathmatchminutes"] = "Minutes to death match!";
			$messages["deathmatchseconds"] = "Seconds to death match!";
			$messages["chestrefill"] = "chest been refilled!";
			$messages["remainingminutes"] = "minutes remaining!";
			$messages["remainingseconds"] = "seconds remaining!";
			$messages["nowinner"] = " §7No winner in the game §8";
			$messages["moreplayers"] = "§aGame>> §fWe need more players to started!";
	...
	$statistic->save();
		...
		if($asassin !== false) {
			...
			foreach($level->getPlayers() as $pl)
			{
				...
				$pl->sendMessage(TextFormat::GRAY . $rankmuerto . "§f" . $muerto . TextFormat::YELLOW . " " . $toUse["kill"] . " " . TextFormat::GRAY . $rankasesino. "§f" . $asesino . TextFormat::YELLOW . ".");
			}
			$statistic = new Config($this->getDataFolder() . "/statistic.yml", Config::YAML);
	...
	public function onCommand(CommandSender $player, Command $cmd, $label, array $args) {
        ...
        switch($cmd->getName()){
			case "hg":
				if($player->isOp())
				{
					if(!empty($args[0]))
					{
						if($args[0]=="make")
						{
							if(!empty($args[1]))
							{
								if(file_exists($this->getServer()->getDataPath() . "/worlds/" . $args[1]))
								{
									...
									$this->mode = 1;
									$player->sendMessage($this->prefix . "Touch anywhere to spawn!");
									$player->setGamemode(1);
									$player->teleport($this->getServer()->getLevelByName($args[1])->getSafeSpawn(),0,0);
								}
								else
								{
									$player->sendMessage($this->prefix . "ERROR world.");
								}
							}
							else
							{
								$player->sendMessage($this->prefix . "ERROR paraments commands.");
							}
						}
						...
						{
							$player->sendMessage($this->prefix . "command invalid");
						}
					}
					else
					{
						$player->sendMessage($this->prefix . "Need more Players.");
					}
				}
				else
				{
			...
			case "lang":
				if(!empty($args[0]))
				{
					if($lang->get($args[0])!=null)
					{
						...
						$playerlang->save();
						$player->sendMessage(TextFormat::GREEN . "Lenguage: " . $args[0]);
					}
					else
					{
						$player->sendMessage(TextFormat::RED . "Lenguage does not exit!");
					}
				}
			return true;
			...
			return true;
                        ...
                        case "hgstart":
                            if($player->isOp())
				{
                                $player->sendMessage("§b10 seconds to starts...");
                                $config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
		...
		$config->save();
                         ...
                         case "hgcf":
                            if($player->isOp())
				{
                                $player->sendMessage("§7starting  in 10...");
                                $config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
		$config->set("arenas",$this->arenas);
		foreach($this->arenas as $arena)
		{
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			if($this->mode==26)
			{
				$tile->setText(TextFormat::GREEN . "Game",TextFormat::GRAY  . "0 / 24",$this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
			...
			else
			{
				...
				{
					if($text[0]==TextFormat::GREEN ."Game")
					{
						$config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
		...
		else if($this->mode>=1&&$this->mode<=24)
		{
			...
			$player->sendMessage($this->prefix . "Spawn " . $this->mode . " has registered!");
			$this->mode++;
			if($this->mode==25)
			{
				$player->sendMessage($this->prefix . "Touch the death match spawn!.");
			}
			$config->save();
		}
		else if($this->mode==25)
		{
			...
			$config->set("arenas",$this->arenas);
			$player->sendMessage($this->prefix . "Touch anysign to register!");
			$spawn = $this->getServer()->getDefaultLevel()->getSafeSpawn();
			$this->getServer()->getDefaultLevel()->loadChunk($spawn->getFloorX(), $spawn->getFloorZ());
			$player->teleport($spawn,0,0);
			$config->save();
		...
		foreach($tiles as $t) {
			if($t instanceof Sign) {	
				...
				if($text[3]==$this->prefix)
				{
					...
					$ingame = TextFormat::GREEN . "Game";
					$config = new Config($this->plugin->getDataFolder() . "/config.yml", Config::YAML);
					if($config->get($text[2] . "PlayTime")!=660)
					{
						$ingame = TextFormat::RED . "Gaming";
					}
					else if($aop>=24)
					{
						$ingame = TextFormat::WHITE . "Full";
					}
					$t->setText($ingame,TextFormat::GRAY  . $aop . " / 24",$text[2],$this->prefix);
				}
			}
		}
	}
						...
						if(count($playersArena)>=2)
						{
							if($timeToStart>0)
							{
								...
								foreach($playersArena as $pl)
								{
									...
									$toUse = $lang->get($playerlang->get($pl->getName()));
									$pl->sendTip(TextFormat::GRAY . $timeToStart . " " . $toUse["seconds"]);
								}
								if($timeToStart<=0)
								{
							...
							else
							{
								...
								if($aop==1)
								{
									...
									$config->set($arena . "StartTime", 180);
                                                                ...
                                                                if(($aop>=2)){
                                                                    foreach($playersArena as $pl)
                                                                        {$pl->sendTip("§aGame>> §7" . $aop . " §fRemaining Players.!");
                                                                        }
                                                                }
								$time--;
                                                                if($time == 659)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage("§aGame>> §fYou are in HungerGames. 30 Seconds to be the game will be started!");
									}
								}
                                                                if($time == 645)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage("§aGame>> §fYou have 15 seconds to be PVP avalable!");
									}
								}
								if($time == 630)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage("§aGame>> §fthe battle is now started. you can now PVP");
									}
								}
                                                                if($time == 500)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage("§aGame>> §fIf you are interested our game you can follow us! @EngineerCraftPE");
									}
								}
                                                                if($time == 420)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage("§aGame>> §fChests been refilled. Pick now!");
										$playerlang = new Config($this->plugin->getDataFolder() . "/languages.yml", Config::YAML);
                                                                                $lang = new Config($this->plugin->getDataFolder() . "/lang.yml", Config::YAML);
										$toUse = $lang->get($playerlang->get($pl->getName()));
										$pl->sendMessage("§aGame>> §fChests been refilled. Pick now!");
								...
								if(is_int($minutes) && $minutes>0)
								{
									foreach($playersArena as $pl)
									{
										$playerlang = new Config($this->plugin->getDataFolder() . "/languages.yml", Config::YAML);
                                                                                $lang = new Config($this->plugin->getDataFolder() . "/lang.yml", Config::YAML);
										$toUse = $lang->get($playerlang->get($pl->getName()));
										$pl->sendMessage("§aGame>> §f" . $minutes . " " . $toUse["deathmatchminutes"]);
									}
								}
								else if($time == 30 || $time == 15 || $time == 10 || $time ==5 || $time ==4 || $time ==3 || $time ==2 || $time ==1)
								{
									foreach($playersArena as $pl)
									{
										$playerlang = new Config($this->plugin->getDataFolder() . "/languages.yml", Config::YAML);
                                                                                $lang = new Config($this->plugin->getDataFolder() . "/lang.yml", Config::YAML);
										$toUse = $lang->get($playerlang->get($pl->getName()));
										$pl->sendMessage("§aGame>> §f" . $time . " " . $toUse["deathmatchseconds"]);
									}
								}
								if($time2 <= 0)
								{
								...
								else
								{
									...
									$lang = new Config($this->plugin->getDataFolder() . "/lang.yml", Config::YAML);
											$toUse = $lang->get($playerlang->get($pl->getName()));
											$pl->sendMessage("§aGame>> §f" . $minutes . " " . $toUse["remainingminutes"]);
										}
									}
									else if($time == 30 || $time == 15 || $time == 10 || $time ==5 || $time ==4 || $time ==3 || $time ==2 || $time ==1)
									{
									...
									$lang = new Config($this->plugin->getDataFolder() . "/lang.yml", Config::YAML);
											$toUse = $lang->get($playerlang->get($pl->getName()));
											$pl->sendMessage("§aGame>> §f" . $time . " " . $toUse["remainingseconds"]);
										}
									}
									if($time <= 0)
									{
										...
										foreach($playersArena as $pl)
										{
											...
											$playerlang = new Config($this->plugin->getDataFolder() . "/languages.yml", Config::YAML);
                                                                                        $lang = new Config($this->plugin->getDataFolder() . "/lang.yml", Config::YAML);
											$toUse = $lang->get($playerlang->get($pl->getName()));
											$pl->sendMessage("§6HungerGames>> ". $toUse["nowinner"] . $arena);
											$pl->getInventory()->clearAll();
										}
										$time = 660;
									}
								...
								foreach($playersArena as $pl)
								{
									foreach($this->plugin->getServer()->getOnlinePlayers() as $plpl)
									{
										$playerlang = new Config($this->plugin->getDataFolder() . "/languages.yml", Config::YAML);
                                                                                $lang = new Config($this->plugin->getDataFolder() . "/lang.yml", Config::YAML);
										$toUse = $lang->get($playerlang->get($plpl->getName()));
										$plpl->sendMessage("§6HungerGames>> " . $pl->getName() . " " . $toUse["won"] . $arena);
									}
									$spawn = $this->plugin->getServer()->getDefaultLevel()->getSafeSpawn();
									$this->plugin->getServer()->getDefaultLevel()->loadChunk($spawn->getX(), $spawn->getZ());
									$pl->getInventory()->clearAll();
